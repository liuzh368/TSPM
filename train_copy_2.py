...
def readPdata(spath=None):
    if not spath:
        spath=os.path.join(setting.workpath,'Pdata_4sq.npy')
    Userdata=np.load(spath,allow_pickle=True)
    return Userdata

def readCountdata(path=None):
    if not path:
        path=os.path.join(setting.workpath,'Count_4sq.npy')
...
for e in range(30):  # 100
    ...
    for i, (x, t, t_slot, s, y, y_t, y_t_slot, reset_h, active_users) in enumerate(dataloader):
        ...
        loss = model_pre(x, y)
        loss_2 = model_time_transfer_pre(x, t_slot, y, y_t_slot)
        lossx = (loss + loss_2) / 2
        print(lossx)
        lossx.backward()
    ...
    if (e+1) % 30 == 0:
        torch.save(model_pre.state_dict(), f"WORK/dyn_network_merge_4sq_{e+1}.pth")
        torch.save(model_time_transfer_pre.state_dict(), f"WORK/dyn_network_time_transfer_merge_4sq_{e + 1}.pth")
        break
