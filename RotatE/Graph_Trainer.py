...
class Graph_Trainer:
    ...
    def create_loc2loc_graph(self,flag):
        ...
        for i in range(self.poi_len):
            ...
            relation_tuple=torch.tensor(np.array([start,relation,end]).transpose(1,0)).cuda(1)
            ...
            if (i+1)%3000==0:
                ...
                if not flag:
                    break
                elif flag and current_ratio < previous_ratio:
                    # 如果当前匹配率低于前一次的匹配率，则提前终止循环
                    print("Early stopping due to decreasing match ratio.")
                    break
